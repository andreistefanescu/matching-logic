...
Inductive IS (S : cfg -> cfg -> Prop) (A : system) : forall (strict : bool) env,
  formula env -> formula env -> Prop :=
  | is_refl : forall env phi, IS S A false env phi phi
  | is_step : forall strict env (phi phi' : formula env),
                (forall e c, phi c e ->
                             (exists c2, S c c2) /\
                             (forall c2, S c c2 -> phi' c2 e)) ->
                 IS S A strict env phi phi'
  | is_axiom : forall strict env phi phi',
                 A env phi phi' -> IS S A strict env phi phi'
  | is_subst : forall strict env env' f phi phi',
       IS S A strict env' phi phi' ->
       IS S A strict env (fun g e => phi g (f e)) (fun g e => phi' g (f e))
  | is_trans : forall strict env phi phi' phi'',
       IS S A strict env phi phi' ->
       IS S A false env phi' phi'' ->
       IS S A strict env phi phi''
  | is_case : forall strict env phi phi1 phi',
       IS S A strict env phi  phi' ->
       IS S A strict env phi1 phi' ->
       IS S A strict env (fun e g => phi e g \/ phi1 e g) phi'
  | is_lf : forall strict env phi phi' psi,
       IS S A strict env phi phi' ->
       IS S A strict env (fun g e => phi g e  /\ psi e)
...
(* use embedding projection pair -
  ...
  *)
  | is_abstr : forall strict env denx
       ...
       IS S A strict (env * denx) phi (fun g rho_and_x => phi' g (fst rho_and_x)) ->
       IS S A strict env (fun g rho => exists x, phi g (rho , x)) phi'
  | is_circ : forall strict env phi phi' phi'',
       IS S A true env phi phi'' ->
       IS S (cons_system env phi phi' A) false env phi'' phi' ->
       IS S A strict env phi phi'.
