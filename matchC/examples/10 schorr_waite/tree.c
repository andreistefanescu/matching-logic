 ...
 * search order and marks the nodes reachable from the root.  However, instead
 * of implementing the traversal recursively or iteratively with a stack, which
 * would take memory and thus defeat its purpose (Schorr-Waite is used for
 * garbage collection), the Schorr-Waite algorithm uses the already existing
 * pointers in the graph to encode the stack.  During its operation, the graph
 * is therefore significantly altered, so it is important to prove that in
 * the end one obtains the original (reachable part of the) graph, but marked.
 *
 * The graphNode structure uses the field mark to track how many times the
 * depth-first search traversal has visited the current node so far.
 * The valid values for mark are:
 ...
 * In other words, the points-to structure of the tree stays unchanged when
 * the function terminates; what changes is only the mark status of the tree
 * (not marked before, but fully marked after).
 * 
 ...
 * pointers of the nodes in the two trees at p and q.  For the definitions
 * and axioms required by this proof see [2].
 *
       ...
       /\ pointers(T) = pointers(?T) */
{
  ...
  while (p != NULL) {
    ...
    // conditional below by pulling the first three assignments before the
    // conditional (don't forget to also modify the condition accordingly).
