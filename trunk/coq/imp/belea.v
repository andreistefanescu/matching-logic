...
case (Zeven_odd_dec n).
intros.
assert (Hn : exists k : Z, n = 2 * k).
apply Zeven_ex. assumption.
elim Hn; intros k Hn2k.
subst n.
replace (2 * k - 1 + 1) with (2 * k).
replace (2 * k * (2 * k + 1)) with (2 * (k * (2 * k + 1))).
replace (2 * (k * (2 * k + 1))) with ((k * (2 * k + 1)) * 2).
rewrite Z_div_mult_full.
replace ((2 * k - 1) * (2 * k)) with ((2 * k - 1) * k * 2).
rewrite Z_div_mult_full.
ring.
unfold not.
intros H20; inversion H20.
ring.
unfold not.
intros H20; inversion H20.
ring.
...
ring.
intros.
assert (Hn : exists k : Z, n = 2 * k + 1).
apply Zodd_ex. assumption.
elim Hn; intros k Hk.
subst n.

replace (2 * k + 1 + 1) with (2 * (k + 1)).
replace ((2 * k + 1) * (2 * (k + 1))) with (((2 * k + 1) * (k + 1)) * 2).
rewrite Z_div_mult_full.
replace ((2 * k + 1 - 1) * (2 * k + 1 - 1 + 1)) with ((k * (2 * k + 1 - 1 + 1)) * 2).
rewrite Z_div_mult_full.
ring.
unfold not; intros Hbelea; inversion Hbelea.
ring.
unfold not; intros Hbelea; inversion Hbelea.
ring.
ring.
...
Theorem example5' :
  forall n' : nat,
    ...
    exists env',
      imp_run
      (Conf example_pgm' (myenv a 0))
      (Conf b env').
  intros.
  replace (a * (a + 1) / 2) with (a * (a + 1) / 2 + 0) in H0.
  subst b.
  eapply example5.
  apply H.
  apply Zplus_comm.
...
Theorem example6 :
  forall n' : nat,
    exists env',
      imp_run
      (Conf example_pgm' (myenv (Z_of_nat n') 0))
      (Conf ((Z_of_nat n') * ((Z_of_nat n') + 1) / 2) env').
  intros.
  eapply example5'.
  reflexivity.
  reflexivity.
