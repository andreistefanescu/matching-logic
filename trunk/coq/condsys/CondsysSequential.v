...
Module MatchLReduction(Base : ObjectLanguage).
  ...
  Definition rho_valid_srule (rule : SimpleRule) (rho : Valuation) (ts : relation M) :=
    forall gamma',
      ...
      exists gamma'',
        (clos M ts false) gamma' gamma'' /\
        Satisfies gamma'' rho (snd rule).
  
  Definition rho_valid_first (rule : SimpleRule) (rho : Valuation) :=
    exists gamma',
      Satisfies gamma' rho (fst rule).
  
  Definition rho_valid_ssys (ssys : SimpleSystem) (rho : Valuation) (ts : relation M) :=
    List.Forall (fun x => rho_valid_srule x rho ts) ssys.
  ...
  Inductive is_suffix { A : Type } : list A -> list A -> Prop :=
  | here :
    forall slist,
      is_suffix slist slist
  | next :
    forall slist blist bhead,
      is_suffix slist blist ->
      is_suffix slist (bhead :: blist)
  .
      
  Notation "x 'suffix_of' y" := (is_suffix x y) (at level 60, right associativity).
      ...
      exists rho : Valuation,
        exists rest : SimpleSystem,
          exists irule : SimpleRule,
            (cons irule rest) suffix_of (snd rule) /\
            Satisfies gamma'' rho (fst (fst rule)) /\
            List.Forall
              (fun srule => rho_valid_srule srule rho (ts csys)) rest /\
...
(* S \models phi' => phi'' = soundness *)
    ...
    Lemma sound_union :
      ...
      unfold  union_ssys.
      intros.
      remember (List.in_app_or A B srule H2) as b.
      inversion b.
      destruct s1; destruct s2; simpl in H1; rewrite H1; try (apply H; assumption); try apply gamma_sound_unstrict; auto.
      destruct s1; destruct s2; simpl in H1; rewrite H1; try (apply H0; assumption); try apply gamma_sound_unstrict; auto.
    Qed.
...
Notation "x 'inCS' y" := (inCS_ x y) (at level 60).
    ...
    Lemma belea' : forall (crule : CondRule),
      ...
      forall srule side_conditions,
        crule = (srule, side_conditions) ->
        forall (rule : SimpleRule) (rest : SimpleSystem),
          (rule :: rest) suffix_of side_conditions ->
          forall gamma',
            opterminates S gamma' ->
            forall rho gamma_i',
              Satisfies gamma' rho (fst srule) ->
              List.Forall (fun srule => rho_valid_srule srule rho (ts S)) rest ->
...
Lemma suffix_weaken : forall { A : Type },
  forall (slist blist : list A),
    forall (shead : A),
      (shead :: slist) suffix_of blist ->
      slist suffix_of blist.
  intros.
...
Lemma suffix_of_implies_in : forall { A : Type },
  forall (first : A) rest list,
    (first :: rest) suffix_of list ->
    List.In first list.
  intros.
...
Proof.
  ...
  specialize (H4 H6).
  unfold gamma_sound_srule in H4.
